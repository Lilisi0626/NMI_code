...
for k in range(len):
    for i in range(np.size(n_patience)):
        ...
        model.add(keras.layers.Dense(units=n_in, activation='sigmoid', input_dim=80))
        ...
        xtrain = np.zeros((n[k], n_in))
        ytrain = np.zeros((n[k], n_out))
        for j in range(n[k]):
        ...
        xvalid = np.zeros((int(n[k] * 0.2), n_in))
        yvalid = np.zeros((int(n[k] * 0.2), n_out))
        ...
        pyplot.xlabel('epoch')
        pyplot.show()
        model.save('nn_model_' + str(n[k]) + '.h5')
        # calculat error
        ...
        csv_writer.writerow([n[k], RMSE, MME, MAE, time2 - time1])
